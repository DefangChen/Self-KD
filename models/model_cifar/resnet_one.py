...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv1x1(inplanes, width)  # 1x1的卷积核，升降维，增加非线性性
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=10, num_branches=3, bpscale=False, avg=False, ind=False,
        ...
        # CIFAR-10是3x32x32通道图像,暂时不晓得inplanes和dilation这俩在干啥
        self.inplanes = 16
        self.dilation = 1

        # 又是一个不晓得在干啥的参数，字面意思好像是用stride代替dilation？但是好像后面完全没有用到这个参数
        ...
        # Applies Batch Normalization over a 4D
        self.bn1 = nn.BatchNorm2d(16)  # 16指的是通道数
        self.relu = nn.ReLU(inplace=True)
        # 制造block
        ...
        #  采用自适应平均池化层 the target output size of the image of the form H x W.
        ...
        # 大概是使用ONE方法，增加池化层，线性层，和BatchNorm层
        if not self.avg:
            self.avgpool_c = nn.AvgPool2d(16)  # 池化窗口设定为16x16
            self.control_v1 = nn.Linear(fix_inplanes, self.num_branches)
            self.bn_v1 = nn.BatchNorm1d(self.num_branches)

        # 对网络中的所有模型进行参数初始化
        # nn.init()函数的作用:Return the recommended gain value for the given nonlinearity function.
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # He初始化参数；选择“ fan_out”将保留反向传递的幅度。
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            # batchnorm2d层有两个参数weights和bias
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck):  # isinstance() 函数来判断一个对象是否是一个已知的类型
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):  # blocks代表block的个数
    ...
    def forward(self, x):
        ...
        # CL
            ...
            return pro, x_m  # x_m应该是ensemble logits，pro应该是每个分类器头的logits
