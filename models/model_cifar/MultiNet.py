...
class StuNet(nn.Module):
    def __init__(self, model="resnet32", num_branches=4, num_classes=10, input_channel=64, en=False, factor=8,
        ...
        for i in range(num_branches):
            if model == "resnet32":
                setattr(self, 'stu' + str(i), resnet32(num_classes=num_classes, KD=True))
            elif model == "resnet110":
                setattr(self, 'stu' + str(i), resnet110(num_classes=num_classes, KD=True))
            elif model == "vgg16":
                setattr(self, 'stu' + str(i), vgg16(num_classes=num_classes, KD=True, dropout=dropout))
            elif model == "densenetd40k12":
                setattr(self, 'stu' + str(i), densenetd40k12(num_classes=num_classes, KD=True))
            elif model == "wide_resnet20_8":
                setattr(self, 'stu' + str(i), wide_resnet20_8(num_classes=num_classes, KD=True))
